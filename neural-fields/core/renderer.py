...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        power = deltas * rays_density
        euler = torch.exp(-power)
        transmittance = torch.cat([torch.ones_like(euler[:, :1]), euler[:, :-1] + eps], dim = 1)
        transmittance = torch.cumprod(transmittance, 1)
    ...
    def _aggregate(
        ...
        to_sum = weights * rays_feature
        feature = torch.sum(to_sum, dim = 1)
    ...
    def forward(
        ...
        for chunk_start in range(0, B, self._chunk_size):
            ...
            # print("Sample points shape:", cur_ray_bundle.sample_points.shape)
            ...
            feature = self._aggregate(weights, feature.view(-1, n_pts, feature.shape[-1]))

            # TODO (1.5): Render depth map
            depth_values = depth_values.view(-1, n_pts, 1)       # shape: (B, N, 1)
            depth = torch.sum(weights * depth_values, dim=1)     # shape: (B, 1)
